...
- **LPテキストの動的編集機能**: 画像とタイトルに加え、各セクションの「説明文（description）」を管理画面から直接編集可能に拡張。
- **セクションタイプの導入**: 各セクションに `hero`, `content`, `feature`, `cta` などのタイプを設定できるようにし、公開ページでのレイアウト（フォントサイズや余白）を自動最適化。
- **リアルタイムプレビューの高度化**: エディタ右側のモバイルシミュレーターにて、入力したテキストやセクションタイプが即座に反映されるようにUXを強化。
- **汎用Server Actionの実装**: 複数のLPセクションフィールドを一括更新できる `updateLpSection` を実装し、将来的なフィールド追加にも柔軟に対応。
