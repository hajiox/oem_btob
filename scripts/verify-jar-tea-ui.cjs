  ...
  try {
    for (const view of viewports) {
      ...
      for (const [label, total] of jarOptions) {
        ...
        const finalTotal = total + 6000
        assert(result.includes('OEM見積もり結果'), `Jar ${label}: result heading missing`)
        assert(result.includes('単価・内訳を見る'), `Jar ${label}: unit breakdown disclosure missing`)
        assert(result.includes('製造条件・賞味期限・ご注意'), `Jar ${label}: conditions disclosure missing`)
        await form.getByRole('button', {name: 'この内容で相談する', exact: true}).waitFor()
        assert(result.includes(`¥${finalTotal.toLocaleString('en-US')}`), `Jar ${label}: wrong total`)
        ...
        assert((await subtotalRow.innerText()).includes(`¥${total.toLocaleString('en-US')}`), `Jar ${label}: subtotal changed`)
        const shippingRow = breakdown.locator(':scope > div').filter({hasText: '送料・発送梱包手数料'}).first()
        assert((await shippingRow.innerText()).includes('¥6,000'), `Jar ${label}: shipping/packing fee line missing`)
        const unitCostRow = form.getByText('1個あたり（送料等別）', {exact: false}).locator('..')
        assert((await unitCostRow.innerText()).includes(`¥${Math.ceil(total / 400).toLocaleString('en-US')}`), `Jar ${label}: unit cost includes shipping fee`)
        ...
        assert((await manufacturingRow.innerText()).includes('¥180'), `Jar ${label}: manufacturing fee missing`)
      ...
      if (view.name === 'desktop') {
        ...
        await next()
        assert((await form.innerText()).includes('OEM見積もり結果'), 'Jar ingredient branch did not reach result')
      }
      ...
      assert(!(await form.innerText()).includes('OEM見積もり結果'), 'Tea without supplied material must not reach result')
      ...
      assert(result.includes('OEM見積もり結果'), 'Tea bulk result heading missing')
      assert(result.includes('単価・内訳を見る'), 'Tea bulk unit breakdown disclosure missing')
      assert(result.includes('製造条件・賞味期限・ご注意'), 'Tea bulk conditions disclosure missing')
      await form.getByRole('button', {name: 'この内容で相談する', exact: true}).waitFor()
      assert(result.includes('¥256,000'), 'Tea bulk plan total missing')
      {
        const breakdown = form.getByTestId('quote-order-breakdown')
        const subtotalRow = breakdown.locator(':scope > div').filter({hasText: '商品小計'}).first()
        assert((await subtotalRow.innerText()).includes('¥250,000'), 'Tea bulk subtotal changed')
        const shippingRow = breakdown.locator(':scope > div').filter({hasText: '送料・発送梱包手数料'}).first()
        assert((await shippingRow.innerText()).includes('¥6,000'), 'Tea bulk shipping/packing fee line missing')
        const unitCostRow = form.getByText('1袋あたり（送料等別）', {exact: false}).locator('..')
        assert((await unitCostRow.innerText()).includes('¥2,500'), 'Tea bulk unit cost includes shipping fee')
      }
      ...
      assert(result.includes('OEM見積もり結果'), 'Tea retail result heading missing')
      await form.getByRole('button', {name: 'この内容で相談する', exact: true}).waitFor()
      assert(result.includes('¥106,000'), 'Tea retail plan total missing')
      {
        const breakdown = form.getByTestId('quote-order-breakdown')
        const subtotalRow = breakdown.locator(':scope > div').filter({hasText: '商品小計'}).first()
        assert((await subtotalRow.innerText()).includes('¥100,000'), 'Tea retail subtotal changed')
        const shippingRow = breakdown.locator(':scope > div').filter({hasText: '送料・発送梱包手数料'}).first()
        assert((await shippingRow.innerText()).includes('¥6,000'), 'Tea retail shipping/packing fee line missing')
        const unitCostRow = form.getByText('1個あたり（送料等別）', {exact: false}).locator('..')
        assert((await unitCostRow.innerText()).includes('¥250'), 'Tea retail unit cost includes shipping fee')
      }
